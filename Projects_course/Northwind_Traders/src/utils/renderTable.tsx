...
export function renderTable<T>(props: RenderTableProps<T>) {
	...
	// Add an empty column header
	headersLabels.push(
		<th key="empty-column-header" className="table__th" style={{ width: '38.41px' }}></th>
	);
	...
	for (let i = 0; i < data.length; i++) {
		...
		// Add an empty column cell
		tds.push(
			<td key={`empty-column-${i}`} className="table__td" style={{ width: '38.41px', backgroundColor: "#FFF" }}></td>
		);
