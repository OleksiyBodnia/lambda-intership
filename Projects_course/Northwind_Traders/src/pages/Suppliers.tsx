import React, { FC, useEffect, useState } from 'react';
import { Pagination } from '../components/Pagination';
import { fetchData } from '../utils/fetch';
import { renderTable } from '../utils/renderTable';
import { TableData } from '../constants/index';
import { SupplierType, SuppliersResponse } from '../types/api';
import { PiArrowArcRightBold } from 'react-icons/pi';

const Suppliers: FC = () => {
  const [currentPage, setCurrentPage] = useState<number>(
    parseInt(location.search?.split('=')[1]) || 1
  );

  const [data, setData] = useState<SuppliersResponse>();
  const [error, setError] = useState<string | null>(null);
  const [loading, setLoading] = useState(false);

  useEffect(() => {
    const getSuppliers = async () => {
      try {
        setLoading(true)
        const data = await fetchData<SuppliersResponse>(
          `/suppliers?page=${currentPage}&pageSize=20`
        );
        // console.log("Suppliers data:", data);
        setData(data);
        setError(null); 
        setLoading(false)
      } catch (err) {
        setLoading(false)
        console.error("Error fetching suppliers:", err);
        setError(err instanceof Error ? err.message : 'An unknown error occurred');
      }
    };

    void getSuppliers();
  }, [ currentPage]);

  if (!data && loading) {
    return <h4>Loadig Suppliers Data</h4>;
  }

  if (error) {
    return <h4>An error has occurred</h4>;
  }

  return (
    <section className="info">
      <header className="info__header">
        <h2 className="info__title">Suppliers</h2>
        <PiArrowArcRightBold size={24} />
      </header>
      <div className="info__main">
        <div className="info__wrapper">
          {data &&
            renderTable<SupplierType>({
              arr: TableData.supplier,
              data: data.data,
            })}
        </div>
        {data && (
          <Pagination
            currentPage={currentPage}
            maxPages={data.pageCount}
            loading={false}
            setCurrentPage={setCurrentPage}
          />
        )}
      </div>
    </section>
  );
};

export default Suppliers;
